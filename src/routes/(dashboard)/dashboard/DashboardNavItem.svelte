<script>
	import { dashboardSidebar, dashboardSidebarCollapsed, userStore } from '$lib/stores.js'
...
{#if item.sublinks}
	<li>
		<button
			...
			<span
				class="flex-1 ml-3 text-left whitespace-nowrap {$dashboardSidebarCollapsed ? 'hidden' : ''}"
				>{item.name}</span>
			<div class={$dashboardSidebarCollapsed ? 'hidden' : ''}>
				<ChevronDownIcon />
			</div>
		</button>
...
{:else}
	<li>
		<a href={item.link} class={buttonClass}>
			...
			<span class="ml-3 {$dashboardSidebarCollapsed ? 'hidden' : ''}">{item.name}</span>
		</a>
	</li>
{/if}
